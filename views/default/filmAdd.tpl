//Цей код містить форму для додавання нового фільму.
//Форма містить поля для введення назви фільму, року випуску, короткого опису, повного опису,
режисера та зображення фільму. Також у формі є набір чекбоксів для вибору жанру фільму.
У коді використовуються класи Bootstrap для відображення форми в більш привабливому вигляді.
Код містить атрибут "name" для кожного поля форми, що вказує на ім'я поля, яке буде використовуватися
для отримання введених даних на стороні сервера при відправці форми методом POST. Окрім того, у полях
"Назва фільму", "Рік випуску", "Короткий опис" та "Опис" використовуються атрибути "required", що вказує на те,
що ці поля повинні бути заповнені перед відправкою форми.
